...
- (AVAssetReaderOutput *)addReader:(AVAssetTrack *)track  withSettings:(NSDictionary*)outputSettings {
    ...
    if ([_reader canAddOutput:reader]) {
        ...
        NSLog(@"Cannot add input reader %d", (unsigned int)kAudioFormatMPEG4AAC);
        reader = nil;
    }
...
- (CGAffineTransform)videoOrientation:(AVAsset*)videoAsset
{
...
//        _videoInput.transform = self.videoTransform;
        CGAffineTransform txf = [self videoOrientation:self.inputAsset];
        _videoInput.transform = txf;//self.inputAsset.preferredTransform;
