...
//    timer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(myProgressBar) userInfo:nil repeats:YES];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    strobeFrame.origin = CGPointMake(150.0f, 15.0f);
    ...
    //Progress Bar
    progressBar = [[UIProgressView alloc] initWithFrame:CGRectMake(0,50, 320, 10.0)];
    ...
    [cancelButton addTarget:self action:@selector(gotoCancel) forControlEvents:UIControlEventTouchUpInside];
    
    [cancelButton setFrame:CGRectMake(0.0f,0.0f, 60,60)];
...
- (void)visionDidStartVideoCapture:(PBJVision *)vision
{
    ...
    self.durationTimer = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(updateDurationWithRecording) userInfo:nil repeats:YES];
}

- (void)visionDidPauseVideoCapture:(PBJVision *)vision
{
    
    if (progressBar.progress == 1.000000)
    {
        progressBar.progress = 0;
//        PBJVision *obj = [[PBJVision alloc] init];
//        [obj endVideoCapture];
        [self _endCapture];
    }
    [_strobeView stop];
...
- (void) updateDurationWithRecording
{
    //if ([[[self captureManager] recorder] isRecording])
    {
        ...
        progressBar.progress = self.duration/self.maxDuration;
        NSLog(@"self.duration %f, self.progressBar %f", self.duration, progressBar.progress);
//        if (progressBar.progress == 0.400000) {
//            [[NSNotificationCenter defaultCenter] postNotificationName:PAPPhotoUnhineCameraDoneButtonNotification object:nil];
//        }
        if (progressBar.progress == 1.000000)
        {
            self.duration = 0;
...
//            
//            if (self.delegate && [self.delegate respondsToSelector:@selector(didCompleteRecordingVideo:)]) {
//                [self.delegate didCompleteRecordingVideo:self];
//            }
        }
    }
//    else
