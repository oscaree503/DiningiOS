...
- (void)queryTopBusinessInfoForTerm:(NSString *)term location:(NSString *)location offset:(NSString*)startoffset completionHandler:(void (^)(NSDictionary *topBusinessJSON, NSError *error))completionHandler {

  NSLog(@"Querying the Search API with term \'%@\' and location \'%@'", term, location);

  //Make a first request to get the search results with the passed term and location
  NSURLRequest *searchRequest = [self _searchRequestWithTerm:term location:location offset:startoffset];
    NSLog(@"%@", searchRequest.URL);
  ...
  [[session dataTaskWithRequest:searchRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
    ...
    if (!error && httpResponse.statusCode == 200) {
      ...
      if ([businessArray count] > 0) {
          completionHandler(searchResponseJSON, nil);
      } else {
...
- (void)queryTopBusinessInfoForTerm:(NSDictionary *)param
                  completionHandler:(void (^)(NSDictionary *jsonResponse, NSError *error))completionHandler {
    //Make a first request to get the search results with the passed term and location
    NSURLRequest *searchRequest = [NSURLRequest requestWithHost:kAPIHost path:kSearchPath params:param];
    NSLog(@"%@", searchRequest.URL);
    NSURLSession *session = [NSURLSession sharedSession];
    [[session dataTaskWithRequest:searchRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
 ...
 */
- (NSURLRequest *)_searchRequestWithTerm:(NSString *)term location:(NSString *)location offset:(NSString*)startoffset {
  NSDictionary *params = @{
                           @"offset" : startoffset,
                           @"term": term,
                           @"location": location,
